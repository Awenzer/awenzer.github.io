...
window.openVideoGallery = function(key) {
  ...
  if (links.length === 0) {
    ...
    links.forEach((link, i) => {
      ...
      if (isConfigured) {
        // Try to build Instagram embed thumbnail from URL
        const isIG = link.url.includes('instagram.com');
        const thumbBg = isIG
          ? `<div style="width:100%;height:100%;background:linear-gradient(135deg,#1a0533,#050510);display:flex;align-items:center;justify-content:center;font-size:28px;opacity:0.4;">◈</div>`
          : `<div style="width:100%;height:100%;background:linear-gradient(135deg,#050510,#0a0a1e);display:flex;align-items:center;justify-content:center;font-size:28px;opacity:0.3;">▶</div>`;
